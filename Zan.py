...
def banner():
	clear()
	wel = '# WELCOME TO FACEBOOK MBF TOOLS Mark Zukerberg'
	...
	tap.add_row('Zidan','https://github.com/Zidan-XD')
...
def login_lagi():
	...
	try:
		...
		suu = mark(sue, style='default')
		sol().print(suu, style='cyan')
		time.sleep(2.5)
		login_lagi()
	except requests.exceptions.ConnectionError:
		li = '# KONEKSI INTERNET BERMASALAH, PERIKSA & COBA LAGI'
		lo = mark(li, style='default')
		sol().print(lo, style='cyan')
		exit()
...
def dump_publik():
	...
	try:
		koh2 = requests.get('https://graph.facebook.com/v2.0/'+pil+'?fields=friends.limit(5000)&access_token='+token).json()
...
def dump_massal():
	...
	for userr in uid:
		try:
			col = requests.get('https://graph.facebook.com/v2.0/'+userr+'/friends?limit=5000&access_token='+token)
